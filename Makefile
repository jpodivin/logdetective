...
server-up:
	$(COMPOSE_ENGINE) -f docker-compose-dev.yaml up --build --force-recreate -d server worker
...
alembic-generate-revision:
	...
	$(COMPOSE_ENGINE) -f docker-compose-dev.yaml build migrate
	...
	$(CONTAINER_ENGINE) run --rm --user $(MY_ID) --uidmap=$(MY_ID):0:1 --uidmap=0:1:999 \
		...
		--entrypoint bash \
		localhost/logdetective/migrate:latest \
		-c "cd /src && python -m alembic upgrade head"
	...
	$(CONTAINER_ENGINE) run --rm -ti --user $(MY_ID) --uidmap=$(MY_ID):0:1 --uidmap=0:1:999 \
		...
		--entrypoint bash \
		localhost/logdetective/migrate:latest \
		-c "cd /src && python -m alembic revision -m \"$(CHANGE)\" --autogenerate"
