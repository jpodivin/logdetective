...
async def validate_request_size(request: Request) -> int:
...
def task_representation(task: TaskAnalysis) -> TaskResponse:
    ...
    if task.state == AnalysisState.DONE:
        ...
        if task.task_type == TaskType.KOJI:
            ...
            external_id = task.task_metadata.get("task_id")
            log_file_name = task.task_metadata.get("log_file_name")
            if external_id is None or log_file_name is None:
                raise RuntimeError("Completed Koji task metadata is incomplete")
            result = KojiResponse(
                task_id=int(external_id),
    ...
    return TaskResponse(
        id=task.task_id,
        taskType=task.task_type.value.lower(),
