...
from logdetective.models import APIResponse, AnalysisRequest, JobHook, KojiAnalysisRequest
...
async def _execute_generic(
    ...
    timeout = aiohttp.ClientTimeout(
        total=float(os.environ.get("LOG_SOURCE_REQUEST_TIMEOUT", "60")),
        connect=3.07,
    )
    ...
    return TaskOutcome(response=response, metadata={})
...
async def _execute_koji(payload_data: dict[str, Any]) -> TaskOutcome:
    ...
    session = ClientSession(
        ...
        opts={"timeout": SERVER_CONFIG.koji.analysis_timeout},
    )
    try:
        async with asyncio.timeout(SERVER_CONFIG.koji.analysis_timeout):
    ...
    return TaskOutcome(
        response=response,
        metadata={"log_file_name": filename, "task_id": payload.task_id},
    )
...
async def _execute_gitlab(
    ...
    try:
        async with aiohttp.ClientSession(
            ...
            response = await process_gitlab_job_event(
                config,
                connection,
                http,
                forge,
