...
def test_task_enum_values_are_uppercase_and_match_database_labels() -> None:
    """Keep Python enum values aligned with SQLAlchemy's PostgreSQL labels."""
...
async def test_gitlab_redelivery_uses_stable_source_identity():
    ...
    async with DatabaseFactory().make_new_db():
        first, created = await TaskAnalysis.admit(
            ...
            payload={"forge": "gitlab.com", "status": "failed"},
            request_size=0,
            deferrable_task=FakeDeferrable(),
        ...
        duplicate, duplicate_created = await TaskAnalysis.admit(
            ...
            owner_token_name=None,
            task_type=TaskType.GITLAB,
            payload={"forge": "gitlab.com", "status": "failed", "duration": 10},
            request_size=0,
            deferrable_task=FakeDeferrable(99),
            endpoint=EndpointType.ANALYZE_GITLAB_JOB,
            source_id=source_id,
...
@pytest.mark.asyncio
async def test_worker_loss_fences_late_publication():
    public_id = uuid4()
    async with DatabaseFactory().make_new_db():
        task, _ = await TaskAnalysis.admit(
            task_id=public_id,
        ...
        assert started is not None
        assert await TaskAnalysis.mark_worker_lost(
